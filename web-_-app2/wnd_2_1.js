...
function wnd2(x,y,w,h,wnd,p,cb,title,controlbox)
{
	...
	if(controlbox==null)
		controlbox=
		[
			["button","-","width:17px;height:17px;font-size:10px;",15,"minimize/restore","w.minimizerestore();"],
			["button","口","width:17px;height:17px;font-size:10px;",15,"maximize/restore","w.maximizerestore();"],
			["button","x","width:17px;height:17px;font-size:10px;",15,"close","w.dispose();"]
		];
	this.param.x=x;
	this.param.y=y;
	this.param.w=w;
	...
	this.restore=function()
	{
		...
		//this.fr.dock=0;
		if(this.onrestore)this.onrestore(this);
		if(this.param.wnd.wnd_on_restore)
		{
			return this.param.wnd.wnd_on_restore(this);
		}
	};
	this.activate=function(disable_restore)
	{
