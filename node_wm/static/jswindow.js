...
function create_mousemove_helper(wnd,cb) {
	var div=wnd.document.createElement("div");
	div.style.cssText="position:absolute;top:0px;left:0px;right:0px;bottom:0px;z-index:2000;";
...
function destroy_mousemove_helper(obj) {
...
function wnd2_helper(wnd)
{
	...
	this.do_move_start=function(w,e) {
		this.mm_helper=create_mousemove_helper(w.param.wnd, function(X,Y){
			...
			}else if(window.wnd2_resizing!=null)
			{
				...
				var ow=X-w.param.p.scrollLeft+getx(w.fr)-w.dx;
				var oh=Y-w.param.p.scrollTop+gety(w.fr)-w.dy;
				var nw=w.dw+ow;
				var nh=w.dh+oh;
				if(nw<150)nw=150;
				if(nh<70)nh=70;
	...
	this.do_move_end=function() {
		if(window.wnd2_moving)window.wnd2_moving.mov=false;
		if(window.wnd2_resizing)window.wnd2_resizing.resizing=false;
		window.wnd2_moving=null;window.wnd2_resizing=null;
		if(this.mm_helper!=null) {
			destroy_mousemove_helper(this.mm_helper);
			this.mm_helper=null;
		}
	};
	wnd.document.body.onmouseup=function(){window.wnd2_helper_i.do_move_end();};
	...
	wnd.document.addEventListener("touchend",function(ev){window.wnd2_helper_i.do_move_end();},false);
}
function wnd2(x,y,w,h,wnd,p,cb,title,controlbox)
{
	...
	this.maximize=function()
	{
		...
		this.fr.style.top=(this.marginTop-1).toString()+"px";
		this.fr.style.left=(this.marginLeft-1).toString()+"px";
		this.fr.style.right=(this.marginRight-1).toString()+"px";
		this.fr.style.bottom=(this.marginBottom-1).toString()+"px";
		this.maximized=true;
		this.minimized=false;
		if(this.param.wnd.wnd_on_maximize)
		{
				...
				this.tr_title=this.d.createElement("tr");
					this.td_title=this.d.createElement("td");
						...
						this.td_title.onmousedown=function(ev)
						{
							...
							if(this.wnd.param.wnd.wnd2_helper_i!=null) {
								this.wnd.param.wnd.wnd2_helper_i.do_move_start(this.wnd,ev);
							}
							var e;
							if(ev)
								e=ev;
...
function browser(w)
{
	...
	this.div.style.cssText="position:absolute;bottom:0px;left:0px;right:0px;top:0px;";
